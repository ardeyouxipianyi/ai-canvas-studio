...
- 选择性融合原项目 `basketikun/chatgpt2api` v1.4.0 的部分能力，重点补强 OpenAI 兼容接口、账号能力识别、图片 token 估算和错误响应格式。
- 增强 `/v1/images/generations`、`/v1/images/edits`、`/v1/chat/completions`、`/v1/responses` 的图片输入输出兼容性，改善 Cherry Studio 等客户端的接入稳定性。
- 账号池新增 Plus、Team、Pro、Codex 等能力识别与筛选逻辑，生图时可结合账号能力、额度、冷却、最近成功失败和并发占用做更准确的选择。
- 新增图片 token / usage 估算与日志统计，图片生成、图片编辑、聊天图片输入等路径会记录更完整的用量信息。该统计为本地估算值，不等同于上游官方账单。
- 增加更接近 OpenAI 格式的结构化错误响应，接口失败时更容易被客户端识别，也更方便在日志中排查问题。
- 改进多图片结果解析、空结果处理、图片任务轮询和结果稳定性，降低“上游已生成但客户端拿不到图片”的概率。
- 画布右上角定位按钮改为“定位到最新节点”，会按节点创建时间选中并居中显示最新节点。
- 底部提示词输入框支持拖拽调整高度，长提示词编辑更方便。

## v2.2.0 重点变化

- 画布工作台顶部工具栏重新整理，常用操作保留在右上角，设置入口改为独立齿轮按钮，次要操作收进更多菜单。
- 左上角画布工作台增加新建画布按钮，画布列表前置，右上角定位按钮改为定位到全部节点中心位置。
- 初始文生图生成后的提示词节点现在可以继续编辑：选中提示词节点后，可在底部编辑框修改提示词、张数和比例，并以副本方式生成新分支。
- 支持双击节点选中相关节点组，按 Delete 批量删除，按 Esc 取消选择，方便快速清理同一批结果或同一条分支。
- 生成新节点、复制提示词节点、复制编辑节点时，会尽量避让已有节点，减少新结果遮挡旧节点。
- 底部编辑框更紧凑：上传改为图标按钮，额度、张数、比例控件统一字号，比例外部不再展示分辨率。
- 设置中心、号池管理、图片管理、日志管理继续统一为更紧凑的管理工作台样式。
- 图片任务进度改为更平滑的感知进度，减少长时间卡在固定百分比的感觉。
