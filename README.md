...
- 模型服务设置收敛为“生图服务”和“反推服务”两个直接配置入口，减少重复列表和默认服务切换步骤。
- 支持用当前未保存的模型服务表单直接测试连接，并在保存时避免覆盖另一个服务槽位的未保存草稿。
- 反推服务支持独立模型字段，画布反推提示词会优先使用反推模型配置。
- 设置页会提示并支持清理不再使用的旧模型服务配置。
- 优化画布与设置页的若干交互细节、请求处理和配置同步逻辑。
