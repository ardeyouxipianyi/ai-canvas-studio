      ...
      <Card className="rounded-2xl border-white/80 bg-white/90 shadow-sm">
        <CardContent className="space-y-6 p-6">
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center justify-end gap-2">
              ...
              <label className="flex items-center gap-2 rounded-xl border border-stone-200 bg-white px-3 py-2 text-sm text-stone-700">
                <Checkbox checked={backupEnabled} onCheckedChange={(checked) => setBackupField("enabled", Boolean(checked))} />
                {backupEnabled ? "已启用" : "已关闭"}
              </label>
            </div>
          </div>
          ...
          {backupEnabled ? (
            <>
              ...
              <div className="space-y-3">
                ...
                {isLoadingBackups ? (
                  ...
                  <div className="rounded-xl bg-stone-50 px-6 py-10 text-center text-sm text-stone-500">暂无远端备份记录。保存配置并执行一次手动备份后会出现在这里。</div>
                ) : (
                  <div className="space-y-3">
                    {backups.map((item) => {
                      ...
                      return (
                        <div key={item.key} className="flex flex-col gap-3 rounded-xl border border-stone-200 bg-white px-4 py-4 md:flex-row md:items-center md:justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Button type="button" variant="outline" className="h-9 rounded-xl border-rose-200 bg-white px-4 text-rose-700" onClick={() => void removeBackup(item.key)} disabled={isDeleting}>
            ...
            <>
              <div className="rounded-xl border border-stone-200 bg-stone-50 px-4 py-4 text-sm leading-6 text-stone-500">R2 备份管理已关闭，连接信息、备份内容和历史操作已折叠。重新启用后会恢复显示这些设置。</div>
        ...
        <DialogContent className="flex max-h-[85vh] max-w-3xl flex-col overflow-hidden rounded-2xl border-white/80 bg-white">
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto pr-1">
            {detailLoading ? (
              ...
              <div className="rounded-xl bg-stone-50 px-6 py-10 text-center text-sm text-stone-500">暂时无法读取备份详情；如果这是加密备份，请确认当前已填写正确的加密口令并先保存配置。</div>
            ) : (
              <>
