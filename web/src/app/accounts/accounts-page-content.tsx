    ...
    const poll = async () => {
      try {
        ...
        if (data.failed > 0) {
          ...
          toast.error(
            `刷新完成：成功 ${data.refreshed} 个，失败 ${data.failed} 个。${refreshError}`,
          );
          return;
        }
    ...
    <>
      <section className="settings-module-toolbar">
        ...
        <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:items-center sm:justify-end">
          ...
          <Button
            variant="outline"
            className="h-10 rounded-xl border-stone-200 bg-white px-4 text-stone-700 hover:bg-stone-50 sm:min-w-[220px]"
      ...
      <section className="rounded-[24px] border border-stone-200 bg-white/95 p-3 shadow-[0_24px_80px_-44px_rgba(15,23,42,0.5)] backdrop-blur">
        <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-6">
          {metricCards.map((item) => {
            const Icon = item.icon;
            const value = summary[item.key];
            return (
              <div key={item.key} className="min-h-[72px] rounded-2xl border border-stone-200 bg-white px-4 py-3 shadow-sm">
                <div className="flex items-center justify-between gap-3">
                  <span className="truncate text-xs font-medium text-stone-500">{item.label}</span>
                  <Icon className="size-4 shrink-0 text-stone-400" />
                </div>
                <div className={cn("mt-2 text-xl font-semibold leading-none tracking-tight", item.color)}>
                  <span className={typeof value === "number" ? "" : "text-sm"}>
            ...
            <CardContent className="space-y-3 p-4">
              ...
              {refreshJob.failed > 0 && refreshJob.errors[0]?.error ? (
                <div className="rounded-xl border border-rose-100 bg-rose-50 px-3 py-2 text-xs leading-5 text-rose-700">
                  刷新提示：{formatAccountRefreshError(refreshJob.errors[0].error)}
                </div>
              ) : null}
            </CardContent>
          </Card>
